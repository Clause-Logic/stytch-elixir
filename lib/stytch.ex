defmodule Stytch do
  ...
  """
  def webauthn_registration(config, user_id, public_key_credential, opts \\ %{})
  ...
  """
  def update_webauthn_registration(config, webauthn_registration_id, name)
      when is_binary(webauthn_registration_id) and is_binary(name) do
    "/webauthn/#{webauthn_registration_id}"
    |> Client.put(config, %{name: name})
  end

  @doc """
    See: https://stytch.com/docs/api/delete-user-webauthn-registration
  """
  def delete_user_webauthn_registration(config, webauthn_id) when is_binary(webauthn_id) do
  ...
  """
  def b2b_authenticate_sso(config, token, opts \\ %{}) when is_binary(token) and is_map(opts) do
    "/b2b/sso/authenticate"
    |> Client.post(config, %{sso_token: token} |> Map.merge(opts))
  end

  @doc """
    See: https://stytch.com/docs/b2b/api/authenticate-oauth
  """
  def b2b_authenticate_oauth(config, token, opts \\ %{}) when is_binary(token) and is_map(opts) do
    "/b2b/oauth/authenticate"
    |> Client.post(config, %{oauth_token: token} |> Map.merge(opts))
  end

  @doc """
    See: https://stytch.com/docs/b2b/api/authenticate-discovery-oauth
  """
  def b2b_authenticate_discovery_oauth(config, token, opts \\ %{}) when is_binary(token) and is_map(opts) do
    "/b2b/oauth/discovery/authenticate"
    |> Client.post(config, %{discovery_oauth_token: token} |> Map.merge(opts))
  end
